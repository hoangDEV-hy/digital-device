...
exports.register = async (req, res, next) => {
  try {
    ...
    const user = await User.create({ fullName, email, phone, password, deviceIp });
    res.json({ success: true, message: 'Registered', data: { id: user.id, email: user.email } });
  } catch (err) { next(err); }
};
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (user.deviceIp && deviceIp && user.deviceIp !== deviceIp) {
      return res.status(403).json({ success: false, message: 'Device IP mismatch' });
    }
    ...
    const accessToken = jwtUtil.signAccessToken({ id: user.id, role: user.role });
    const refreshToken = jwtUtil.signRefreshToken({ id: user.id });
    await RefreshToken.create({ token: refreshToken, userId: user.id, expiresAt: jwtUtil.getRefreshExpiryDate() });

    res.json({ success: true, message: 'Logged in', data: { accessToken, refreshToken } });
  } catch (err) { next(err); }
};
...
exports.refreshToken = async (req, res, next) => {
  try {
    ...
    const accessToken = jwtUtil.signAccessToken({ id: payload.id, role: payload.role });
